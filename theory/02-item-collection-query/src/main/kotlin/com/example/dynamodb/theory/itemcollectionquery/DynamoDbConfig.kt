...
data class DynamoDbProperties(
    ...
    val tableName: String = "theory_02_item_collection_query", // 이번 Query 예제에서 사용할 table 이름이다.
 ...
 * service는 이 bean을 주입받아 DynamoDB에 item을 저장하고 Query를 호출한다.
 */
@Configuration
class DynamoDbConfig {
    ...
    fun dynamoDbClient(properties: DynamoDbProperties): DynamoDbClient {
        val builder = DynamoDbClient.builder()
            // Query도 특정 AWS region의 DynamoDB endpoint로 요청을 보낸다.
            .region(Region.of(properties.region))

        if (properties.endpoint.isNotBlank()) {
            // local profile에서는 실제 AWS 대신 DynamoDB Local endpoint로 요청한다.
        ...
        if (properties.useDummyCredentials) {
            builder.credentialsProvider(
                StaticCredentialsProvider.create(
                    // DynamoDB Local은 credential 값을 검증하지 않으므로 dummy 값을 넣는다.
