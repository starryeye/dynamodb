...
- 예제 프로젝트의 핵심 코드는 주석만 읽어도 이번 주제가 무엇을 배우는지 알 수 있어야 한다.
- 각 theory 주제의 주요 service/test에는 주제 목표, 핵심 용어 정의, 확인 흐름을 짧게 적는다.
- 예를 들어 table, item, key처럼 처음 나오는 개념은 코드 주석 안에서 먼저 정의한다.
...
- 자세한 이론 설명은 `docs/theory/*.md`를 읽도록 안내하되, 코드만 열어도 최소 맥락은 잡히게 한다.
